...
class Installer:
    ...
    def get_version(self):
        ...
        if os.path.exists(POETRY_LIB):
            with open(os.path.join(POETRY_LIB, "poetry", "__version__.py"), encoding="utf-8") as f:
    ...
    def _make_lib(self, version):
        ...
        print("  - Downloading {} ({:.2f}MB)".format(colorize("comment", name), size / 1024 / 1024))
        ...
        with temporary_directory(prefix="poetry-installer-") as dir_:
            ...
            if checksum != sha.hexdigest():
                raise RuntimeError(
                    "Hashes for {} do not match: {} != {}".format(name, checksum, sha.hexdigest())
                )
    ...
    def display_post_message(self, version):
        ...
        if WINDOWS:
            ...
            poetry_home_bin = POETRY_BIN.replace(os.getenv("USERPROFILE", ""), "%USERPROFILE%")
        elif "fish" in SHELL:
