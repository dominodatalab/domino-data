...
class DB2NativeConfig(Config):
    ...
    username: Optional[str] = _cred(elem=CredElem.USERNAME)
    password: Optional[str] = _cred(elem=CredElem.PASSWORD)
    dsn: Optional[str] = _cred(elem=CredElem.DSN)
...
@attr.s(auto_attribs=True)
class NetAppVolumeConfig(Config):
    """NetAppVolumeConfig datasource configuration."""

    snapshot_version: Optional[str] = _config(elem=ConfigElem.SNAPSHOTVERSION)
    subfolder: Optional[str] = _config(elem=ConfigElem.SUBFOLDER)
...
DATASOURCES = {
    ...
    DatasourceDtoDataSourceType.NETAPPVOLUMECONFIG: "ObjectStoreDatasource",
}
