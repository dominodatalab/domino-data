...
def get_content_size(
    url: str, headers: Optional[Dict[str, str]] = None, http: Optional[urllib3.PoolManager] = None
) -> int:
...
class BlobTransfer:
    ...
    def _load_state(self) -> None:
        ...
        try:
            with open(self.resume_state_file) as f:
                ...
                self._completed_chunks = {
                    tuple(chunk) for chunk in state.get("completed_chunks", [])
                }
    ...
    def _get_ranges_to_download(self) -> List[Tuple[int, int]]:
        ...
        return [
            chunk_range for chunk_range in all_ranges if chunk_range not in self._completed_chunks
        ]
