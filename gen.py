...
class {{ config }}(Config):
    ...
    {% for field_name in datasource_configs[config]["fields"] -%}{% if datasource_configs[config]["fields"][field_name].isOverridable %}
    {{ datasource_configs[config]["fields"][field_name].name }}: Optional[str] = _config(elem=ConfigElem.{{datasource_configs[config]["fields"][field_name].name | upper}}){% endif %}{% endfor %}
    {% for auth_type in datasource_configs[config]["authTypes"] -%}{% for auth_field in auth_configs[auth_type]["fields"]-%}{% if auth_configs[auth_type]["fields"][auth_field].isOverridable %}
    {{ auth_configs[auth_type]["fields"][auth_field].name }}: Optional[str] = _cred(elem=CredElem.{{auth_configs[auth_type]["fields"][auth_field].name | upper}}){% endif %}{% endfor %}
    {% endfor %}
{% endfor %}
'''
)
